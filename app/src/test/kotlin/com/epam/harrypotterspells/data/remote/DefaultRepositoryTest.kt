...
class DefaultRepositoryTest {
    ...
    private val remoteSpells = StubList.spells.drop(2).map { it.toSpannedSpell() }
    private val testString = "test"
    private val lastSpellId = remoteSpells.last().id

    private lateinit var repository: Repository
    private lateinit var testObserver: TestObserver<List<SpannedSpell>>

    @Before
    fun setup() {
        MockKAnnotations.init(this)
        repository = DefaultRepository(api, TestSchedulerProvider())
